...
<script>
  ...
  function agregarDetalle() {
    ...
    msjError = validarTamañoMaximoYNulidad(msjError, 'descripcion', {{ App\Utils\Configuracion::tamañoMaximoDescripcion }}, 'Descripción')
    msjError = validarTamañoMaximoYNulidad(msjError, 'codigoPresupuestal', {{ App\Utils\Configuracion::tamañoMaximoCodigoPresupuestal }},
      'Código Presupuestal del item');
    ...
    for (let item = 0; item < detalleReq.length; item++) {
      ...
      var fila = `  <tr class="selected" id="fila` + item + `" name="fila` + item + `">
                        ...
                        <td style="text-align:center;">
                            <input type="text" class="form-control" name="colTipo` + item + `" id="colTipo` + item + `" value="` + element
        .tipo + `" readonly>
                        </td>
                        <td style="text-align:center;">
                            <input type="text" class="form-control" name="colCantidad` + item + `" id="colCantidad` + item + `" value="` +
        element.cantidad + `" readonly>
                        </td>
                        <td>

                            <input type="text" class="form-control" name="colDescripcion` + item + `" id="colDescripcion` + item +
        `" value="` + element.descripcion + `" readonly>
                        </td>



                        <td style="text-align:center;">
                        <input type="text" class="form-control" name="colCodigoPresupuestal` + item + `" id="colCodigoPresupuestal` +
        item + `" value="` + element.codigoPresupuestal + `" readonly>
                        </td>
                        <td style="text-align:center;">
