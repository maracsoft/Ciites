...
<div class="CSS_SEP">
  ...
  <button id="{{ $button_id }}" onclick="JS_SEP_toggle()" type="button" role="switch" aria-checked="true"
    class="baseButton button{{ $initialClass }}">
    <!-- Enabled: "translate-x-5", Not Enabled: "translate-x-0" -->
    <span aria-hidden="true" id="{{ $circle_id }}" class="circleBase circle{{ $initialClass }}"></span>
  </button>

  <input type="{{ App\Utils\Configuracion::getInputTextOHidden() }}" name="{{ $name }}" id="{{ $inputValue_id }}"
    value="{{ $initialValue }}">
