...
<div id="divEnteroArchivo">
  ...
  <input type="{{ App\Utils\Configuracion::getInputTextOHidden() }}" name="{{ $filenames_input_name }}" id="{{ $names }}" value="">
  ...
  <input type="file" multiple class="" name="{{ $file_input_name }}[]" id="{{ $file_name }}"
    style="{{ App\Utils\Configuracion::getDisplayNone() }}" onchange="onFileInput_{{ $r }}()">
