...
<div class="col-12 col-sm-4">
  ...
  <select class="form-control" id="{{ $cb_dep_name }}" name="{{ $cb_dep_name }}" onchange="clickSelectDepartamento{{ $name }}()">
    ...
    @foreach ($listaDepartamentos as $departamento)
      <option value="{{ $departamento->getId() }}" @if ($codDepartamentoSeleccionado == $departamento->getId()) selected @endif>
        {{ $departamento->nombre }}
      </option>
    @endforeach
...
<div class="col-12 col-sm-4">
  ...
  <select class="form-control" id="{{ $cb_pro_name }}" name="{{ $cb_pro_name }}" onchange="clickSelectProvincia{{ $name }}()">
    ...
    @foreach ($listaProvinciasDelDep as $prov)
      <option value="{{ $prov->getId() }}" @if ($codProvinciaSeleccionada == $prov->getId()) selected @endif>
        {{ $prov->nombre }}
      </option>
    @endforeach
...
<div class="col-12 col-sm-4">
  ...
  <select class="form-control" id="{{ $cb_dis_name }}" name="{{ $cb_dis_name }}"
    onchange="clickSelectDistrito{{ $name }}(this.value)">
    <option value="-1">-- Distrito --</option>
    @foreach ($listaDistritosDeProv as $distr)
      <option value="{{ $prov->getId() }}" @if ($codDistritoSeleccionado == $distr->getId()) selected @endif>
        {{ $distr->nombre }}
      </option>
    @endforeach
...
<input type="{{ App\Utils\Configuracion::getInputTextOHidden() }}" value="{{ $codDistritoSeleccionado }}" name="{{ $output_name }}"
  id="{{ $output_name }}">
