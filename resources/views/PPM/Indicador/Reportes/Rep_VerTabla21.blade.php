@php
  ...
  if($descargarExcel){
    ...
    header("Content-Disposition: attachment; filename=Reporte de Indicador 2.1 .xls");
...
<table border="1" style="{{$subtitulo}}">
  <tbody>
    <tr>
      <td style="{{$textCenter}}" colspan="12">  
        Indicador 2.1: Al finalizar el proyecto, el 80% del grupo meta (70 mujeres y 80 hombres) implementan mecanismos para la adaptación, gestión de riesgos y efectos del cambio climático.
      </td>
    </tr>
  </tbody>
</table>
  ...
  <tbody>
    ...
    @forelse($listaRelaciones as $relacion)
      ...
      <tr>
        ...
        <td>
          {{$relacion->ind21_descripcion_mecanismos}}
        </td>
        <td>
          {{$relacion->ind21_estado_implementacion}}
        </td>
        <td>
          {{$relacion->ind21_beneficiarios}}
        </td>
