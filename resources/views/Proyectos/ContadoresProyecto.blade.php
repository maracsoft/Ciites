...
@section('contenido')
  <div class="card-body">
    ...
    <form id="frmContador" name="frmContador" role="form" action="{{ route('GestionProyectos.agregarContador') }}" method="post">
      @csrf
          ...
          <tr>
            ...
            <td>
              <a href="#" class="btn btn-danger btn-sm btn-icon icon-left" title="Eliminar registro"
                onclick="clickEliminar({{ $item_relacion->getId() }},'{{ $itemcontador->getNombreCompleto() }}','{{ $proyecto->nombre }}')">
                Eliminar
              </a>
            </td>
          </tr>
    ...
    <div class="d-flex m-2">
      ...
      <a href="{{ route('GestionProyectos.Listar') }}" class="btn btn-primary icon-left" style="">
        Regresar
      </a>
    </div>
...
@section('script')
  <script>
    function clickEliminar(codProyectoContador, nombre, proyecto) {
      confirmarConMensaje("Confirmación", "¿Seguro que desea eliminar al contador " + nombre + " del proyecto " + proyecto + "?", "warning",
        function() {
          location.href = "/GestionProyectos/eliminarContador/" + codProyectoContador;
        })
