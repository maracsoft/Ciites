...
@section('contenido')
  ...
  <form method = "POST" onsubmit="" action = "{{ route('SolicitudFondos.Empleado.update', $solicitud->codSolicitud) }}" id="frmsoli"
    name="frmsoli" enctype="multipart/form-data">
      ...
      <div class="row">
              ...
              <div class="col"> {{-- Combo box de banco --}}
                <select class="form-control" id="ComboBoxBanco" name="ComboBoxBanco">
                  ...
                  @foreach ($listaBancos as $itemBanco)
                    <option value="{{ $itemBanco['codBanco'] }}" @if ($solicitud->codBanco == $itemBanco->codBanco) selected @endif>
        ...
        <div class="col-md"> {{-- COLUMNA DERECHA --}}
            ...
            <div class="col-sm-9"> {{-- Combo box de proyecto --}}
              <select class="form-control" id="ComboBoxProyecto" name="ComboBoxProyecto" onchange="actualizarCodPresupProyecto()">
                ...
                @foreach ($listaProyectos as $itemProyecto)
                  <option value="{{ $itemProyecto['codProyecto'] }}" @if ($solicitud->codProyecto == $itemProyecto->codProyecto) selected @endif>
          ...
          <div class="row mt-1">
            ...
            <div class="col-sm-4"> {{-- Combo box de monedas --}}
              <select class="form-control" id="ComboBoxMoneda" name="ComboBoxMoneda">
                ...
                @foreach ($listaMonedas as $itemMoneda)
                  <option value="{{ $itemMoneda->codMoneda }}" @if ($solicitud->codMoneda == $itemMoneda->codMoneda) selected @endif>
                    {{ $itemMoneda->nombre }}
                  </option>
                @endforeach
              </select>
            ...
            <div class="col-sm-4"> {{-- Combo box de monedas --}}
              <input class="form-control" id="codigoContrapartida" name="codigoContrapartida"
                value="{{ $solicitud->codigoContrapartida }}">
            </div>
