...
<head>
  <title>Declaración Jurada {{ $DJ->codigoCedepas }}- Gastos de Movilidad</title>
  ...
  <div id="principal" style="width: 635px; height: 750px">
      ...
      <thead style="width:100%">
        ...
        <tr>
          <th colspan="4" style="text-align: center; background-color: rgb(0, 102, 205); color: white">
            DECLARACIÓN JURADA - GASTOS MOVILIDAD
          </th>
        </tr>
      </thead>
      <tbody>
    ...
    <table style="width:100%">
      <tbody style="width:100%;">
        <tr>
          <td colspan="4">Yo, <b>{{ $DJ->getEmpleado()->getNombreCompleto() }}</b>, identificado/a con DNI
            <b>{{ $DJ->getEmpleado()->dni }}</b> domiciliado/a en <b>{{ $DJ->domicilio }}</b> DECLARO BAJO
            JURAMENTO haber realizado gastos de los cuales no tengo documentos sustentatorios hasta por la suma de
            <b>{{ $DJ->getMoneda()->simbolo }} {{ number_format($DJ->importeTotal, 2) }}</b> tal como detallo a continuación:
          </td>
        </tr>
        <tr style="font-weight: bold; background-color:rgb(238, 238, 238);">
          <td style="width: 80px; text-align: center;">Fecha</td>
          <td style="width: 100px; text-align: center;">Lugar</td>
        ...
        @foreach ($listaItems as $itemdetalle)
          <tr>
            ...
            <td style="text-align: center">{{ $itemdetalle->lugar }}</td>
            <td style="text-align: center">{{ $itemdetalle->detalle }}</td>
            <td style="text-align: right">{{ number_format($itemdetalle->importe, 2) }}</td>
          </tr>
        @endforeach

        <tr>
          <td colspan="2" class="sinlineas"></td>
