      ...
      <div class="card-tools">
        <ul class="nav nav-pills ml-auto">
          <li class="nav-item">
            ...
            <button href="#" class="btn btn-success" onclick="limpiarModal()" data-toggle="modal" data-target="#ModalSede">
            ...
            <tr>
              ...
              <td>
                ...
                <select class="form-control select2 select2-hidden-accessible selectpicker"
                  onchange="cambiarAdministrador({{ $itemSede->codSede }})" data-select2-id="1" tabindex="-1" aria-hidden="true"
                  data-live-search="true" id="comboAdministrador{{ $itemSede->codSede }}">

                  @foreach ($listaAdministradores as $administrador)
                    <option value="{{ $administrador->codEmpleado }}"
                      {{ $itemSede->codEmpleadoAdministrador == $administrador->codEmpleado ? 'selected' : '' }}>
              ...
              <td>
                <button href="" class="btn btn-info" onclick="clickEditarSede({{ $itemSede->codSede }})" data-toggle="modal"
                  data-target="#ModalSede">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form id="frmSede" name="frmSede" action="{{ route('Sede.GuardarEditar') }}" method="POST">
            <input type="{{ App\Utils\Configuracion::getInputTextOHidden() }}" name="codSede" id="codSede" value="0">
            @csrf
...
<script>
  ...
  function validarFormularioSede() {
    ...
    tamañoMaximoNombreSede = {{ App\Utils\Configuracion::tamañoMaximoNombreSede }}
