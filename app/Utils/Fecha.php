...
class Fecha
{
  ...
  public static function formatoFechaParaVistasEscrita($fecha)
  {
    ...
    $codMes = date("n", strtotime($fecha));
    $dia = date("j", strtotime($fecha));
    $mes = Mes::findOrFail($codMes);
    $nombre_mes = $mes->nombre;

    $nombre_dia_español = static::traducirDiaAEspañol($nombre_dia_ingles);

    $fecha_restante = date("Y", strtotime($fecha));
    return $nombre_dia_español . " " . $dia . " " . $nombre_mes . " " . $fecha_restante;
  }
